...
static void bordearmapa(int x,int y,int **m,int material){
    for(int i= 0; i<y; i++){
        for(int j= 0; j<x; j++){
...
static void random(int *x, int max, int min){
    (*x)= (rand()%(max-min+1))+min;
}
static void rellenar(int nx,int ny,int *c,int **m, int material){
    if(m[ny][nx]==VACIO) (*c)++;
    m[ny][nx]= material;
}
